...
The `/demo` route and `/?demo=1` query entry both enter demo mode directly. The query entry is replaced with the canonical `/demo` URL before the service worker registers, so offline reloads use one stable cache key. Any `license` query parameter on either demo entry is removed without storage access or verification. Both start on the box drill and visibly list two realistic sample results: 82/100 and 76/100.
